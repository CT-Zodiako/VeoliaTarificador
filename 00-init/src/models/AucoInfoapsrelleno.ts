...
@Index("IXAUCO_INFOAPSRELLENO01", ["apsaId", "rellId", "iareAnno", "iareMes"], {
  unique: true,
})
@Index("PK_AUCO_INFOAPSRELLENO", ["iareId"], { unique: true })
@Entity("AUCO_INFOAPSRELLENO")
export class AucoInfoapsrelleno {
  ...
  @ManyToOne(
    () => AucoApsrelleno,
    (aucoApsrelleno) => aucoApsrelleno.aucoInfoapsrellenos
  )
  @JoinColumn([
    { name: "APSA_ID", referencedColumnName: "apsaId" },
    { name: "RELL_ID", referencedColumnName: "rellId" },
  ])
  aucoApsrelleno: AucoApsrelleno;
}
